...
KW_mapping={"name":"name","source":"pdb","organism":"organism",
            ...
            "binding_partners": "partners_name", "biounit":"",
            "selection":"", "pvec_0":"principalVector",
            "pvec_1":"principalVector", "pvec_2":"principalVector",
            "offset_0":"offset", "offset_1":"offset", "offset_2":"offset"}

arrays = ["principalVector", "offset"]
...
class JsonRecipeParser:
    ...
    def parseJsonString(self, astring):
        """Parse cpecified with 'filename'  json file. Add ingredients and compartments to the database"""
        jsondic = json.loads(astring)
        self.parseJson(jsondic)

    def parseJson(self, jsondic):
        rootid = None
    ...
    def getIngredientData(self, ingr_dic, is_surface=False):
        ...
        #group_id
        if "Type" in ingr_dic and ingr_dic["Type"] == "Grow":
